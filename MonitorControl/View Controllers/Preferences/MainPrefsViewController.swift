...
class MainPrefsViewController: NSViewController, SettingsPane {
  ...
  func populateSettings() {
    ...
    if #available(macOS 13.0, *) {
      startAtLogin = SMAppService.loginItem(identifier: helperID).status == .enabled
    } else {
      startAtLogin = (SMCopyAllJobDictionaries(kSMDomainUserLaunchd).takeRetainedValue() as? [[String: AnyObject]])?.first { $0["Label"] as? String == helperID }?["OnDemand"] as? Bool ?? false
    }
